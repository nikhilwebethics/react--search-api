...
function Posts(){
  ...
  const url = (`https://renemorozowich.com/wp-json/wp/v2/posts?filter[posts_per_page]=10&page=${page}&search=${query}&_embed`);
...
<NavBar/>
 <h1> POSTS: </h1> <br/>
  ...
  {isLoading? (<div className="text-center mt-10">
        ...
        {!post.length && (<div className="text-center mt-10">
                <div className="spinner-border animate-spin inline-block w-8 h-8 border-4 rounded-full" role="status">
                <span className="visually-hidden">Loading...</span>
                </div>
        </div>) }
